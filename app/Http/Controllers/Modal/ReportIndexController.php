...
class ReportIndexController extends Controller
{
    ...
    function DownloadReportV2(Request $request){
        $category = DB::table('question')
            ->join('category', 'question.id_category','category.id_category')
            // ->join('comment', 'category.id_category','comment.id_category')
            // ->join('page', 'page.id_page','category.id_page')
        ...
        if ($comment->count() > 0) {
            ...
            foreach ($result as $k => $p) {
                ...
                //echo "HELLO DUNIA";
                foreach($comment as $key=>$come){
                    //echo "TemP Start : ".json_encode($tempPageId);

                    if ($p->id_page == $come->id_page) {
                            // echo $key;
                            // echo "P". json_encode($p->name);
                            // echo '<br>';
                            // echo "C". json_encode($come->name);
                            // echo '<br>';

                        if ($key == 1 && $p->name != $come->name) {
                          
                            
                            $rowRate = $come->rate;
                            $rowTitle = $come->name;
                            // echo json_encode($rowRate);
                            // echo '<br>';
                            // echo json_encode($rowTitle);
                            // echo '<br><br>';

                                if ($tempPageId == 0) {
                                    $tempPageId = $come->id_page;
                                }

                        }else if($key > 1  && $p->name != $come->name){
                            ...
                            // echo '<br>';
                            // echo '<br>';
                            // echo "Tem HERE : ".json_encode($tempPageId);
                            // echo '<br>';
                            // echo "TemP : ".json_encode($come->id_page);
                            // echo '<br>';

                            if ($tempPageId == 0 && $p->name != $come->name) {
                                $rowRate = $come->rate;
                                $rowTitle = $come->name;
                                if ($tempPageId == 0) {
                                    $tempPageId = $come->id_page;
...
/*                foreach($comment as $key=>$come){
        ...
        return view('admin.download.report-download-next', compact('page','respondent', 'category','answer', 'comment', 'result'));
    ...
    public function StoreImage(Request $request)
    {
        ...
        $file = $folderPath . "respondent_" .$id_respondent . '_'.$version.'.jpg';
